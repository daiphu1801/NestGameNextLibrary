...
export default function RandomPlayPage() {
    ...
    const spinRandomGame = useCallback(() => {
        ...
        // Simulate slot machine spinning effect
        let spinCount = 0;
        const maxSpins = 15;
            ...
            {!isLowPerformanceMode && (
                <div className="fixed inset-0 pointer-events-none -z-10 overflow-hidden">
                    ...
                    <div
                        ...
                        style={{
                            background: 'radial-gradient(circle, rgba(0, 245, 212, 0.18) 0%, rgba(0, 212, 255, 0.08) 40%, transparent 70%)',
                            filter: 'blur(80px)',
            ...
            <div className="container mx-auto px-4 lg:px-8 py-12">
                ...
                <div className="flex flex-col items-center mb-12 text-center">
                    <div className={cn(
                        ...
                        "bg-gradient-to-br from-primary/20 to-accent/20 ring-primary/30",
                        ...
                        <div className="relative flex flex-col items-center">
                                ...
                                {selectedGame ? (
                                    <>
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-4">
                                            <h3 className={cn(
                                                "font-bold text-white text-lg truncate transition-all",
                                                isSpinning && "blur-sm"
                                            )}>
                                                {selectedGame.name}
                                            </h3>
                                            <p className="text-white/70 text-sm">
                                                {selectedGame.category || 'NES Game'}
                                            </p>
                                        </div>
                                    ...
                                    <div className="w-full h-full bg-gradient-to-br from-secondary to-muted flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-muted-foreground text-sm text-center px-4">
                                            {t('random.clickToStart') || 'Bấm nút để bắt đầu'}
                                        </p>
                                    </div>
                                )}
                            </div>
                            ...
                            {showResult && selectedGame && (
                                <div className="flex items-center gap-2 mb-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                                    ...
                                    <span className="text-lg font-bold text-gradient-magic">
                                        {t('random.yourGame') || 'Game dành cho bạn!'}
                                    </span>
                                    <Sparkles className="w-5 h-5 text-yellow-400" />
                                </div>
                            )}
                            ...
                            <div className="flex flex-col sm:flex-row items-center gap-4">
                                ...
                                <button
                                    ...
                                    disabled={isSpinning || allGames.length === 0}
                                    className={cn(
                                        "group relative px-10 py-4 rounded-2xl font-bold text-lg uppercase tracking-wider transition-all duration-300",
                                    ...
                                    <span className="flex items-center gap-3">
                                        {isSpinning ? (
                                            <>
                                                ...
                                                {t('random.spinning') || 'Đang quay...'}
                                            </>
                                        ) : (
                                            <>
                                                <Dices className="w-6 h-6 group-hover:rotate-180 transition-transform duration-500" />
                                                {t('random.spin') || 'QUAY NGẪU NHIÊN'}
                                            </>
                                        )}
                                    </span>
                                </button>

                                {/* Play Button - Show when result is displayed */}
                                {showResult && selectedGame && (
                                    <button
                                        onClick={handlePlayGame}
                                        className={cn(
                                            "px-8 py-4 rounded-2xl font-bold text-lg uppercase tracking-wider transition-all duration-300",
                                            "bg-white/10 border border-white/20 text-foreground",
                                            "hover:bg-primary/20 hover:border-primary/50 hover:scale-105",
                                            "active:scale-95 animate-in fade-in slide-in-from-left-4"
                                        )}
                                    >
                                        <span className="flex items-center gap-3">
                                            <Play className="w-5 h-5 fill-current" />
                                            {t('random.playNow') || 'CHƠI NGAY'}
                                        </span>
                                    </button>
                                )}
                            </div>
                            ...
                            <div className="mt-8 flex items-center gap-6 text-sm text-muted-foreground">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>{history.length} {t('random.played') || 'đã quay'}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
// Game Image Component with fallback
        ...
        return (
            <div className="w-full h-full bg-gradient-to-br from-secondary to-muted flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground px-4 line-clamp-2">{game.name}</p>
                </div>
            </div>
        );
    }
