...
import { Sparkles, Loader2, StarOff, Gamepad2, ChevronLeft, ChevronRight } from 'lucide-react';
import { adminService } from '@/services/adminService';
import { useToast } from '../components/ToastProvider';

export default function MustPlayGamesPage() {
    ...
    const loadGames = useCallback(async () => {
        setLoading(true);
        try {
            const data = await adminService.getMustPlayGames(page, SIZE);
            setGames(data.content);
            setTotal(data.totalElements);
        } catch (err: any) {
            showToast('error', err.message || 'Lỗi khi tải game Chắc Hẳn Bạn Đã Chơi');
        } finally {
            setLoading(false);
        }
    }, [page, showToast]);
    ...
    const handleRemoveMustPlay = async (game: any) => {
        if (!confirm(`Bạn có chắc muốn bỏ đánh dấu bộ sưu tập tuổi thơ cho game "${game.name}"?`)) return;
        setProcessingId(game.id);
        try {
            await adminService.toggleMustPlay(game.id);
            setGames(prev => prev.filter(g => g.id !== game.id));
            setTotal(t => t - 1);
            showToast('success', `Đã bỏ đánh dấu game ${game.name}`);
        } catch (err: any) {
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-white flex items-center gap-3">
                        ...
                        Game Chắc Hẳn Bạn Đã Chơi
                    </h1>
                    <p className="text-[#A5B4CB] text-sm mt-1">Danh sách tinh chọn gợi ý "Những tựa game tuổi thơ" ở màn hình công cộng thay cho Leaderboard.</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="rounded-[10px] p-12 flex flex-col items-center justify-center text-center" style={{ background: '#24303F', border: '1px dashed #2E3A47' }}>
                    <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4" style={{ background: 'rgba(168,85,247,0.1)' }}>
                        <Sparkles className="w-8 h-8 text-[#A855F7]" />
                    </div>
                    <h3 className="text-white font-semibold text-lg mb-2">Danh sách đang trống</h3>
                    <p className="text-[#A5B4CB] text-sm max-w-sm">Hãy vào trang Quản lý Games và chọn "Chắc hẳn đã chơi" đối với những tựa game hấp dẫn nhất nhé.</p>
                </div>
            ) : (
                <>
                            ...
                            <div key={game.id} className="group rounded-[12px] overflow-hidden relative transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl" style={{ background: '#24303F', border: '1px solid #2E3A47' }}>
                                ...
                                <div className="aspect-[4/3] w-full relative bg-[#1C2434] overflow-hidden border-b" style={{ borderColor: '#2E3A47' }}>
                                    ...
                                    <div className="absolute top-3 left-3 flex gap-2">
                                        <span className="px-2.5 py-1 rounded-full text-[10px] items-center gap-1 font-bold shadow bg-white/10 backdrop-blur-md text-[#A855F7] border border-[#A855F7]/30 flex">
                                    ...
                                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                        <button
                                            onClick={() => handleRemoveMustPlay(game)}
                                ...
                                <div className="h-1 w-full bg-gradient-to-r from-[#8B5CF6] via-[#A855F7] to-[#D946EF] opacity-70"></div>
                            </div>
                        ))}
                    </div>
