...
export interface J2mePlayerHandle {
  exportSave: () => Promise<{ state: Blob; thumbnail?: Blob } | null>;
  importSave: (stateBlob: Blob) => Promise<void>;
}

const SAVE_TIMEOUT = 10000; // 10s timeout for save/load operations

/**
 * J2ME Player – renders the J2ME-For-Web emulator inside an iframe.
 * Exposes exportSave/importSave via ref for the save/load modal.
 */
export const J2mePlayer = forwardRef<J2mePlayerHandle, J2mePlayerProps>(
  function J2mePlayer({ gameUrl, onLoaded, isMobile }, ref) {
    ...
    // ================================================================
    // Save/Load via postMessage bridge
    // ================================================================
    useImperativeHandle(ref, () => ({
      async exportSave(): Promise<{ state: Blob; thumbnail?: Blob } | null> {
        ...
        return new Promise((resolve) => {
          ...
          function handler(e: MessageEvent) {
            if (e.data?.type === 'j2me-save-data') {
              clearTimeout(timeout);
              window.removeEventListener('message', handler);
              // Convert JSON string to Blob for the save system
              const stateBlob = new Blob([e.data.data], { type: 'application/json' });
              let thumbnailBlob: Blob | undefined;
              if (e.data.thumbnail) {
                // thumbnail is a data URL, convert to blob
                const byteStr = atob(e.data.thumbnail.split(',')[1]);
                const arr = new Uint8Array(byteStr.length);
                for (let i = 0; i < byteStr.length; i++) arr[i] = byteStr.charCodeAt(i);
                thumbnailBlob = new Blob([arr], { type: 'image/jpeg' });
              }
              resolve({ state: stateBlob, thumbnail: thumbnailBlob });
            } else if (e.data?.type === 'j2me-save-error') {
              clearTimeout(timeout);
              window.removeEventListener('message', handler);
              resolve(null);
            }
          ...
          iframe.contentWindow!.postMessage({ type: 'j2me-export-save' }, '*');
        });
      },

      async importSave(stateBlob: Blob): Promise<void> {
        const iframe = iframeRef.current;
        if (!iframe?.contentWindow) throw new Error('Emulator not ready');
        ...
        return new Promise((resolve, reject) => {
          ...
          function handler(e: MessageEvent) {
            if (e.data?.type === 'j2me-import-done') {
              clearTimeout(timeout);
              window.removeEventListener('message', handler);
              resolve();
            } else if (e.data?.type === 'j2me-import-error') {
              clearTimeout(timeout);
              window.removeEventListener('message', handler);
              reject(new Error(e.data.error || 'Import failed'));
            }
          }

          window.addEventListener('message', handler);
          iframe.contentWindow!.postMessage({ type: 'j2me-import-save', data: text }, '*');
        });
      },
    }), []);
