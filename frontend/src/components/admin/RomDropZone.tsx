...
export function RomDropZone({ onUploaded }: RomDropZoneProps) {
    ...
    const handleFile = async (file: File) => {
        ...
        if (!['nes', 'sfc', 'smc', 'gba', 'md', 'gen', 'bin', 'zip', 'swf', 'jar', 'jad', 'iso', 'cso', 'chd', 'sms', 'gg', 'pce', 'ngc', 'ngp', 'a26', '32x', 'vb', 'ws', 'wsc'].includes(ext || '')) {
            setError('Định dạng không được hỗ trợ. Hãy kiểm tra lại đuôi file.');
            return;
        }
        setError('');
        setUploading(true);
    ...
    return (
        <div className="space-y-3">
            ...
            >
                <input ref={inputRef} type="file" accept=".nes,.zip,.sfc,.smc,.gba,.md,.gen,.bin,.swf,.jar,.jad,.iso,.cso,.chd,.sms,.gg,.pce,.ngc,.ngp,.a26,.32x,.vb,.ws,.wsc" className="hidden" onChange={onInputChange} />
                ...
                {uploading ? (
                    ...
                    <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-[#637381] text-xs">Hỗ trợ .nes · .sfc · .gba · .md · .zip · .swf · .jar · .iso · .cso · thêm nhiều đuôi retro khác...</p>
                    </div>
                )}
            </div>
